...
func (h *UserHandler) RegisterSubmit() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		_, err := h.store.UsersByUsername(form.Username)
		if err == nil {
		...
		if err := h.store.CreateUser(&grc.User{
			ID:       uuid.New(),
		...
		h.sessions.Put(r.Context(), "flash", "Your registration was sucessful, please log in.")
...
func (h *UserHandler) LoginSubmit() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		user, err := h.store.UsersByUsername(form.Username)
		if err != nil {
